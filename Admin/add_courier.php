...
if(isset($_POST['submit'])) {
  ...
  //  shipment info
   ...
   $sql = "INSERT INTO `parcels` (`consignment`, `username`, `sender name`, `sender phone`, `sender email`, `sender city`, `sender zip code`, `sender street`, `sender state`, `reciever name`, `reciever phone`, `reciever email`, `receiver city`, `receiver zip code`, `receiver street`, `receiver state`, `courier type`, `courier weight`, `comment`, `payment`, `Status`, `date`) VALUES ('$consignment', '', '$name', '$phone', '$email', '$city', '$zip', '$street', '$state', '$recname', '$recphone', '$recemail', '$reccity', '$reczip', '$recstreet', '$recstate', '$couriertype', '$courierweight', '$couriercomment', '$payment', 'To be approved', current_timestamp());";

   $result = mysqli_query($conn, $sql);
   
   if($result) {
       echo "<div class='alert alert-success' id='alert' role='alert'>
       <strong>Your courier has been placed!</strong>!</button>
     </div>";
   }
   else {
...
<head>
  ...
  <link rel="stylesheet" href="../Admin/CSS Files/add_courier.css">
  <title>Add courier</title>
</head>
    ...
    <form action="" method="POST">
      ...
      <div class="item">
        ...
        <div class="city-item">
          ...
          <input type="text" name="ship-state" placeholder="state" required />
      ...
      <input type="submit" name="submit" value="Place courier" class="btn btn-primary">
    </form>
  </div>
  <script>
    ...
    function alert() {
      ...
      location.href="http://localhost/darcel/admin/admin_panal.php"
    }
  </script>
</body>
